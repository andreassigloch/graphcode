...
type ShimResponse = { id: number; ok: true; result: unknown } | { id: number; ok: false; error: string };
...
export async function callHost(socketPath: string, tool: string, input: unknown): Promise<unknown> {
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('data', (chunk) => {
      ...
      socket.end();
      try {
        const res = JSON.parse(buffer.slice(0, nl)) as ShimResponse;
    ...
    socket.write(JSON.stringify({ id, tool, input } satisfies ShimRequest) + '\n');
  });
}
