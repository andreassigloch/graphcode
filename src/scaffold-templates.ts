...
export function guardrailsContent(): string {
  return [
    ...
    '- **The URL lives in `docs/views/dashboard.url`.** GVE writes its ACTUAL bound',
    '  address there on startup and removes it on shutdown. Read that file — never',
    '  assume a port: it is dynamic (Vite bumps on conflict). File absent = not running.',
