...
export function guardrailsContent(): string {
  return [
    ...
    '  bound address there on startup and removes it on shutdown. Never assume a port: it is',
    '  dynamic (Vite bumps on conflict), so a stale file can point at a NEIGHBOUR repo\'s',
    '  viewer — `status` resolves that by asking `api/dashboard` which repo is served.',
