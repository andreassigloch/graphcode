...
export async function runExecutor(opts: RunExecutorOptions): Promise<ExecutorStats> {
  ...
  const runPreflight = async (
    input: unknown,
  ): Promise<{ effective: unknown; blocked: MutateOutcome | null }> => {
    const parsed = registry['graph_mutate'].inputSchema.safeParse(input);
    if (!parsed.success) return { effective: input, blocked: null };
    try {
      ...
      if (pf.action === 'fixed') {
        ...
        return { effective: pf.input, blocked: null };
      }
      return { effective: parsed.data, blocked: null };
    } catch (err) {
  ...
  const callGate = async (input: unknown): Promise<MutateOutcome> => {
    try {
