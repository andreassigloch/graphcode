...
export async function runExecutor(opts: RunExecutorOptions): Promise<ExecutorStats> {
  ...
  const runMutate = async (input: unknown): Promise<MutateOutcome> => {
    // Batch-Preflight (CR-GC-284): Batch-Hygiene VOR dem Gate — kein zweites
    // Gate-Urteil; bei jedem Preflight-Fehler geht der Batch unverändert durch.
    let effective = input;
    try {
      const pf = preflightBatch(input, await loadPreflightKnown());
