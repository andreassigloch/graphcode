...
import type { GraphcodeAuditEntry } from './audit-file.js';
 ...
 * and a torn tail are skipped (same tolerance as FileAuditLog) — but the file is
 * never touched (a foreign worktree's log is read-only to the merge).
 */
export function readBranchLog(logPath: string, sinceVersion: number): GraphcodeAuditEntry[] {
  const raw = readFileSync(logPath, 'utf8');
  const entries: GraphcodeAuditEntry[] = [];
  for (const line of raw.split('\n')) {
    if (!line.trim()) continue;
    let parsed: GraphcodeAuditEntry & { checkpoint?: boolean };
    try {
      parsed = JSON.parse(line) as GraphcodeAuditEntry & { checkpoint?: boolean };
    } catch {
