/**
 * host.ts — graphcode HOST process + READ-ONLY SSE bridge (MOD-surface).
 *
 * The bridge exposes a strictly READ/live-view HTTP surface over the single
 * disk Kuzu store of a repo (REQ-single-kuzu-owner). WHO owns that store is
 * decided in `kernel/own-kuzu.ts` (FUNC-own-kuzu-host, CR-GC-447) — not here;
 * this file is the transport above it. Two modes (CR-GC-237): OWN — `graphcode
 * host` wins the election and owns the harness itself; ATTACH — the elected MCP
 * host injects ITS harness, so live viewer and agent sessions coexist (the
 * bridge follows the lock):
 *
 ...
 * not a config flag. The WRITE path is MCP-stdio only (surface/mcp-server.ts — agents
...
export class HostBridge {
  ...
  /**
   * Take the one harness over disk Kuzu (`ownKuzu`), wire its update-event
  ...
  async start(): Promise<number> {
    ...
    // WER den Store besitzt, entscheidet der kernel (`ownKuzu`, CR-GC-447) —
    // OWN öffnet genau einen, ATTACH übernimmt den bereits gewählten. Die Bridge
    // stellt nur den Scope (repo-abgeleitete Member-Identität, wie serveStdio)
    // und ihre SSE-Senke.
    ...
    this.harness = owned.harness;
    this.ownsHarness = owned.owns;
    ...
    if (path.startsWith('/context/')) {
      ...
      try {
        ...
        sendJson(res, 200, {
          ...
          formatE: SE_FORMAT_E_CODEC.serialize(slice, { omitProvenance: true }),
        });
      } catch {
