...
export function bindWriteTools(ctx: ToolContext): MCPToolRegistry {
  ...
  /**
   * Format-E-Block → MutateCommands (CR-GC-276, CR-GC-627). Ein Input-Codec, KEIN zweiter Schreibweg.
   *
   ...
   *
   * `decode()` bleibt unangetastet — es ist der LESEweg (Rundlauf, `graph_export`), und
   * Encode-Ausgabe kennt nur `+`.
   *
   * Was hier weiterhin geprüft wird, weil `decode()` es prüfte:
  ...
  const formatEToCommands = (text: string): { commands: MutateCommand[]; unnamed: string[] } => {
    ...
    const diff = ctx.gcCodec.inner.parse(text, { resolveType });
    ...
    const typPruefen = (uid: string, type: string | undefined): string => {
      if (!type) {
        throw new Error(
          `graph_mutate: node "${uid}" carries no type — Format-E v2 declares it in a "### <TYPE>" section`,
        );
      }
      const storeType = resolveType(uid);
      if (storeType !== undefined && storeType !== type) {
        throw new Error(
          `graph_mutate: node "${uid}" is declared as "${type}" but exists as "${storeType}" — ` +
      ...
      switch (op.type) {
        ...
        case 'remove_node': {
          ...
          if (resolveType(uid) === undefined) {
            throw new Error(
              `graph_mutate: delete rejected — node "${uid}" does not exist; a deletion must name a node the store holds.`,
            );
          }
        ...
        case 'remove_edge': {
          ...
          for (const [rolle, uid] of [['source', sourceId], ['target', targetId]] as const) {
            if (!angelegt.has(uid) && resolveType(uid) === undefined) {
              throw new Error(
                `graph_mutate: implicit-add rejected — ${rolle} "${uid}" not present; ` +
        ...
        case 'merge_nodes': {
          ...
          if (ids.length !== 2) {
            throw new Error(
              `graph_mutate: a merge line absorbs exactly ONE node into ONE other ("M source + target"); ` +
          ...
          for (const uid of ids) {
            if (!angelegt.has(uid) && resolveType(uid) === undefined) {
              throw new Error(`graph_mutate: merge rejected — "${uid}" is neither declared here nor in the store.`);
            }
          }
        ...
        default:
          throw new Error(`graph_mutate: unknown Format-E operation "${(op as { type: string }).type}"`);
      }
    }
    ...
    if (knotenKollision.length > 0) {
      throw new Error(
        `graph_mutate: the same uid is deleted AND written in one Batch: ${knotenKollision.join(', ')}. ` +
    ...
    if (kantenKollision.length > 0) {
      throw new Error(
        `graph_mutate: the same edge is deleted AND added in one Batch: ${kantenKollision.join(', ')}. ` +
