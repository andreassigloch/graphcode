...
export function guardrailsContent(): string {
  return [
    ...
    '  unsettled order) goes into the model **as an assumption** — an Assumption Review entry',
    '  (`se-irr`), a REQ with an open target value, or an ACTOR with an open channel — and is named',
    '  in your closing message. Ask a question only when a human can answer it: in a headless run',
    '  nobody does, and a question there is a dead end (CR-GC-592).',
    '',
