...
### 8.6  `TEST-attributvertraege` — Attributvertraege gelesen ueber den Familienleser

Vertragstest der Attributvertraege testRefs und realRef auf der Konsumentenseite: gebunden, ungueltig und fehlend haben je ihre eigene Aussage (graph_tests, Verifikationsbericht, Code-Fakten). Dass kein graphcode-Leser den Vertrag selbst parst, prueft RC-09 in TEST-code-conformance.

verify ▶ `SCHEMA-real-ref` · `SCHEMA-test-refs` · testRefs: `tests/attributvertraege.test.ts`

### 8.7  `TEST-audit-retention` — Audit-Aufbewahrungs-Abnahme
...
### 8.8  `TEST-audit-rules-passed` — Bestandene Regeln im Audit
...
### 8.9  `TEST-audit-trail-projection` — Audit-Trail als schlanke Projektion
...
### 8.16  `TEST-cli-run` — Abnahme des run-Verbs
...
### 8.21  `TEST-create-harness-smoke` — Produktionsverdrahtung der Fabrik
...
### 8.22  `TEST-dashboard-ontology-sync` — Dashboard-Ontologie-Test
...
### 8.23  `TEST-dashboard-readonly` — Dashboard-Panels-Abnahme
...
### 8.24  `TEST-deny-stale-read` — Lesesperre gegen veraltete Dokumente
...
### 8.25  `TEST-distribution` — Self-contained npx distribution (automated)
...
### 8.26  `TEST-doc-export` — Doc-Re-Export-Test
...
### 8.27  `TEST-docs-taxonomy` — Docs-Taxonomie-Inspektion
...
### 8.28  `TEST-edge-only-batch` — Reiner Kanten-Batch ohne Typ-Sektion
...
### 8.29  `TEST-executor-bestofn` — Best-of-N-Auswahl im Treiber
...
### 8.30  `TEST-executor-config-contract` — Vertragstest der Lauf-Konfiguration
...
### 8.31  `TEST-executor-preflight` — Batch-Hygiene vor dem Gate
