...
Exception to the source, not to the rule: `regel-matrix.{md,csv}` is generated by
`node scripts/regel-matrix.mjs` from the rule catalogs (contracts, se-engine, graphcode), not from
the graph.
