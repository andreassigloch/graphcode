...
| **AD-8** | **Graph→Markdown Re-Export** verpflichtend (`UC-doc-export`/`FUNC-export-markdown`): human-readable Views deterministisch generiert, GENERATED-Header, nie hand-editiert. **Gebaut (CR-GC-113/220):** 16 deterministische Views. `lean`-Scope **verfeinert** (CR-GC-226): Renders = Projektionen (keine separaten Deliverables), Creations (ConOps/FMEA/Assumption/Trade/Impl-Plan) = leichtgewichtige Urteils-Inputs + Gate-Vorbedingung. Pointer: `docs/archive/proposals/readiness-artifact-model.md` §3. | Schließt die graph-is-ssot-Drift-Lücke; Renders nie hand-editiert, Creations leichtgewichtig in-scope (kein ASIL-D). |
