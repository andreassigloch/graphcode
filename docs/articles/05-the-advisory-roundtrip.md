...
*Five of the six are built and live: ① read, ② status, ③ propose, ④ apply — modeled in graphcode's
own graph as `FCHAIN-advisory-roundtrip`, composing the real functions behind each step — plus
⑤ measure, which ships as the gate's fit advisory: every successful mutation (and every dry run)
carries the Architecture-Fitness Δ across all six components and names any regression, without
blocking. ⑥ report is the tool response, not a separate function.*
...
④ apply     the one Apply-Gate                             → rule-checks the edit for real
   ...
   │
   ▼
⑤ measure   ADVISOR, after the edit — measures, never blocks
...
| ⑤ measure| resistance-to-change improved, but self-containment (one of the six fitness numbers) got slightly worse — reported as a named regression, not a block |
...
measuring, not just describing. `REQ-advisory-roundtrip-latency` states a 200ms target at the
spike's reference size (382 elements / 785 traces; graphcode's own model has since grown to 497
elements / 1,041 traces). `REQ-responsiveness` covers a narrower, older case — draft-apply on just
the affected subgraph, not the whole round.

Measured (`SPIKE-GC-advisory-roundtrip-latency.md`): the round missed the target out of the box
(363ms) — not from any single slow step, but from `suggestEdits` (③ propose) doing a full
...
re-deriving them). That fix is live here since the shared packages moved to pinned registry
versions; re-measured 2026-08-15 against the shipped `@sigloch/se-optimizer@0.4.0`: **191ms**
median at reference size.
