# CR-GC-217: `graph_authoring_guide` — Meta-Modell-Kanten-Guidance für graph-natives Autoren

**Status:** Open (2026-06-27) · **Milestone:** `MS-5-efficiency` · **Max Files:** 3
**Graph (SSOT):** zu seeden (gate-only) `REQ-authoring-guide`, `FUNC-graph-authoring-guide` (→ `src/mcp-tools.ts`), `TEST-graph-authoring-guide` (→ `tests/mcp.context.test.ts`), `CR-GC-217`; unter `MS-5-efficiency`.
