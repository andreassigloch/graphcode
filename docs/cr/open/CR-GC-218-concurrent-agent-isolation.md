...
**Status:** DRAFT — DEFERRED, no action now (2026-06-27). Captures the discussion for
later; nothing to implement until revisited.
...
- then committed a **second `CR-GC-217`** (`graph-authoring-guide`) on top of CR-217's
  `graph-state-time-travel` → two files now share number 217.
...
`graph-authoring-guide`). Resolve by renumbering one (suggest the later
`graph-authoring-guide` → next free number) — a coordination call with the other
session, intentionally not done unilaterally here.
