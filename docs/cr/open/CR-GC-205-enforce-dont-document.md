...
**Status:** In Progress — **Items 1 + 2 + 3 + 4 done** (2026-06-21); **Item 5 (FUNC code binding / R-20)** contracts-seitig in Parallel-Session committed-pending, graphcode-Consumer-Update (R-20→CDR-Partition) hier erledigt · **Milestone:** `MS-5-efficiency` · **Datum:** 2026-06-21 · **Max Files:** 5 (split per item)
...
- **Item 5 (Mechanismus committed, Daten/LSP offen):** R-20 (FUNC-code-binding, warning) — contracts-Regel + `CodeRefSchema` + `FUNC.codeRef/external/concept` + `MOD.path` (Versions 3.6.0/2.3.0) **committed `8b1a99d`**; graphcode-Consumer-Update R-20→**CDR**-Partition **committed `0d7f537`**. **Rest-Arbeit der Parallel-Session:** `codeRef`-Backfill auf alle realisierten FUNC-Knoten (analog zum testRef-Backfill in Item 4 — bis dahin feuert R-20 als Warning auf jeder ungebundenen FUNC, Compliance bleibt 100% weil warning) + die LSP-gestützte Symbol-/Cross-Module-Call-Konformanz. **→ CR-205 schliesst erst, wenn diese Rest-Arbeit landet.**
