 ...
 *
 * Real disk Kuzu in mkdtemp, durable FileAuditLog beside the store, no mocks.
 */
...
import type { GraphcodeAuditEntry } from '../src/audit-file.js';
...
describe('TEST-occ (CR-GC-233): graphVersion on reads, baseVersion check on writes', () => {
  ...
  it('a STALE baseVersion is rejected (tier block) with the delta of applied batches since', async () => {
    ...
    // Delta-report: BOTH intermediate applied batches, with their commands (what changed, not just "stale").
    expect(stale.occ?.staleBaseVersion).toBe(0);
    expect(stale.occ?.delta.map((e) => e.graphVersion)).toEqual([1, 2]);
    expect(stale.occ?.delta.every((e) => (e.commands?.length ?? 0) >= 1)).toBe(true);
  ...
  it('graph_realize honours baseVersion identically (stale → reject + delta, fresh → applies)', async () => {
    ...
    expect(stale.success).toBe(false);
    expect(stale.occ?.delta).toHaveLength(1);
