...
describe('TEST-graph-context: upstream spec-closure (CR-GC-213)', () => {
  ...
  const fixture = {
    elements: [
      { id: 'FN-1', type: 'FUNC', name: 'Slice', description: 'recall-first slicer (no codeRef yet)' },
  ...
  it('flags FN-1 in missingRefs — FUNC has no codeRef/referenceImpl to implement from', async () => {
    const registry = bindToolsToHarness(harness);
...
describe('TEST-graph-context: missingRefs is empty when the FUNC carries a codeRef', () => {
  let tmp: string;
  let harness: GraphCodeHarness;
    ...
    elements: [
      {
        ...
        type: 'FUNC',
        name: 'Realized func',
        description: 'has codeRef',
        codeRef: { file: 'src/slice.ts', symbol: 'slice' },
      },
