...
import { registerAutoExport, AUTO_EXPORT_DEBOUNCE_MS, type AutoExportHandle } from '../src/projections/auto-export.js';
...
describe('CR-GC-392: Shutdown im Entprell-Fenster laesst kein unvollstaendiges Artefakt zurueck', () => {
  ...
  it('der Host verdrahtet den flush in den Shutdown — nicht nur die Funktion existiert', () => {
    ...
    const src = readFileSync(join(__dirname, '..', 'src', 'surface', 'mcp-server.ts'), 'utf8');
