 ...
 *
 * graph_realize binds a FUNC's codeRef (and optionally a TEST's testRef) in one
...
// A realized FUNC with no codeRef (R-20) and a realized TEST with no testRef (R-19) — both warnings,
...
describe('TEST-graph-realize (CR-GC-216): flat realize affordance through the gate', () => {
  ...
  it('sets codeRef through the gate; the FUNC leaves missingRefs; the delta is returned', async () => {
    ...
    // The codeRef is actually on the node, set through the gate (not a side store).
    const fn = harness.getGraph().nodes.find((n) => n.uid === 'FN-x')!;
    expect(fn.attributes.codeRef).toEqual({ file: 'src/x.ts', symbol: 'doX' });
  });
