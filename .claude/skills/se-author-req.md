...
2. A concept-level TEST node: `{ "op": "add-node", "node": { "uid": "TEST-<slug>", "type": "TEST", "name": "<what it checks>", "description": "<target + tool + pass/fail constraint — the test CONCEPT, not code>" } }`. Once the runnable test exists, set `attributes.testRef { file, tool, level }` so `graph_tests` can select it (CR-GC-134).
