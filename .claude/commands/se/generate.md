...
3. **Der `prompt` ist deine Arbeitsanweisung.** Führe genau sie aus:
   ...
   - Alternativen als `graph_mutate {formatE, dryRun:true}` einreichen; Verdicts in dieser Rangfolge vergleichen — `block` verwerfen, dann `steeringDelta` der Fokus-Dimension, dann `steerAdvisory.improvement` (entschärft der Zug die schlimmste Stelle?), dann `tier`. `fitAdvisory` ist nur Bericht: es nennt echte Umstrukturierungen Regression (CR-GC-483/583). Jeder Preview wird auditiert (Vorschlag→Verdict) — auch verworfene Kandidaten sind Evidenz.
